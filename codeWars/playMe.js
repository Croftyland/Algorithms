...
Are you bored or tired of coding in the same way as usual? Here, meet the CodeWarsRunner game. You don't have to play it by yourself.
Instead, your AI will do it for you! Very simple, just press keys, run, and reach goals in all stages. That's it!
Task

You have to write an AI, AI(game) function, to receive an object of arcade game framework, TooGameCWR. Your AI should play and solve
all stages available. Key pressing, game inspecting, and game status are managed by the framework API. For more information, please
see in the API listing below.
Game Briefing:
    ...
    A goal on floor: area 10 units
        Goal reach attempt must be done within goal area, otherwise you die. Successful attempt will automatically proceed you to the
        next level.
...
CodeWarsRunner Framework API:
    ...
    TooGameCWR.press(KeyStroke)
        Input KeyStroke as string : "KnKnKn..."
            K as character (see in Key Mapping) followed by n as number of time in second, can be either integer or float. The
            command execution will be terminated if the stage is cleared or when you die.
...
Note 2: If all stages are cleared, TooGameCWR.inspectTG() and TooGameCWR.press(KeyStroke) will return "All Cleared!". Conversely,
it will be "Game Over!" if all lives have been consumed.
...
*/
function AI(game){
    let status = game.status(), speed = 10, g = game.inspectTG()
    while (g !== 'All Cleared!') {
        let location = g.objects.pop().location[0],
            duration = Math.abs(location) / speed,
            keystroke = (location < 0 ? '←' : '→') + duration + 'X1'
        game.press(keystroke)
        g = game.inspectTG()
    }
}
